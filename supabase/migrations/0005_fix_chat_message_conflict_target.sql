-- `session_id` is also an output parameter of the RPC. Use a named unique
-- constraint rather than a column-list conflict target so PL/pgSQL never has
-- to resolve that identifier.
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'uq_chat_messages_external_message'
      and conrelid = 'public.chat_messages'::regclass
  ) then
    alter table public.chat_messages
      add constraint uq_chat_messages_external_message
      unique using index uq_chat_messages_external_message;
  end if;
end;
$$;
...
begin
  ...
  insert into public.chat_sessions (
    ...
    p_session_key, nullif(lower(trim(p_email)), ''), now(), left(p_message, 300), p_session_metadata
  ...
  if (
    select count(*) from public.chat_messages as chat_message
  ...
  ) on conflict on constraint uq_chat_messages_external_message do nothing
