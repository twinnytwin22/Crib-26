  ...
  ({ className, ...props }, ref) => {
    ...
    return (
      <button
        ...
        className={cn(
          "absolute inset-y-0 z-20 hidden w-4 -translate-x-1/2 transition-all ease-linear after:absolute after:inset-y-0 after:left-1/2 after:w-0.5 group-data-[side=left]:-right-4 group-data-[side=right]:left-0 hover:after:bg-sidebar-border sm:flex",
...
>(({ className, showIcon = false, ...props }, ref) => {
  // Use deterministic width on server to prevent hydration mismatch; adjust after mount.
  const [width, setWidth] = React.useState<string>('70%');
  React.useEffect(() => {
    // Client-only randomization (optional aesthetic) without breaking hydration.
